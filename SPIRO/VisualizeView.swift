...
struct VisualizeView: View {
    @State private var data: [VisualData] = []
                ...
                for row in worksheet.data?.rows.dropFirst(1) ?? [] {
                    ...
                    if let idString = row.cells[0].value,
                       ...
                       let trial = Int(tString) {
                        excelData.append(VisualData(index: index, id: id, visit: visit, trial: trial))
                    }
                }
    ...
    var body: some View {
        VStack {
            ...
            List(data, id: \.index) { item in
                NavigationLink(destination: AnimeView(item: item)) {
                    HStack {
